...
void drawScene1(void)
{
    ...
    glm::mat4 translate = glm::translate(model, glm::vec3(R, 0.0f, 0.0f));
    glm::mat4 rotate = glm::rotate(model, glm::degrees(spinAngle), spinAxis);
...
void drawScene2(void)
{
	...
	// camera setup 
    glm::mat4 view = glm::lookAt(glm::vec3(R * cos(spinAngle), 1.0f, R * sin(spinAngle)),   //eye = camera position
						              glm::vec3(0.0f, 0.0f, 0.0f),   //center = point we look at (view direction  = eye-center)
						              glm::vec3(0.0f, 1.0f, 0.0f) ); //up vektor (camera orientation along the view direction)
...
void drawScene3(void)
{
    ...
    moveDirection.x = cos(viewAngle);
    moveDirection.y = 0;
    moveDirection.z = sin(viewAngle);
...
void drawScene4(void)
{
    ...
    viewDirection.x = cos(viewAngle) * cos(beta);
    viewDirection.y = sin(beta);
    viewDirection.z = sin(viewAngle) * cos(beta);

    // upVector
    glm::vec3 upVector = glm::vec3(-cos(viewAngle) * sin(beta), cos(beta), -sin(viewAngle) * sin(beta));
...
void initTask(int taskNumber)
{
  switch(taskNumber) {
	...
	case 3:
		...
		position.y = 0.5f;
		position.z = 5.0f; 
        viewAngle = -M_PI/2.0f;   // used for moveDirection computation
		break;
	case 4:
...
void mySpecialKeyboard (int key, int x, int y) {
  ...
  switch (key) {
	...
	case GLUT_KEY_RIGHT:  // rotate the viewDirection left
	  ...
	  viewAngle += VIEW_DIRECTION_ANGLE_DELTA;
      viewAngle = (viewAngle > 2 * M_PI) ? (viewAngle - 2 * M_PI) : viewAngle;
	...
	case GLUT_KEY_UP:  // step forward
	  // BEGIN OF SOLUTION - TASK 3 and 4 ========================
      position += (STEP_SIZE * moveDirection);
	  // END OF SOLUTION - TASK 3 and 4 ========================
	  break;
	...
	case GLUT_KEY_PAGE_UP: // raising up the view direction
	  // BEGIN OF SOLUTION - TASK 4 ========================
	  beta += VIEW_DIRECTION_ANGLE_DELTA;
