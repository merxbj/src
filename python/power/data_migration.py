...
def import_data(to_db: mariadb, table, columns):
    ...
    for row in from_table:
        ...
        for column in columns:
            formated_cell = datetime.datetime.fromisoformat(row[column]).strftime('%Y-%m-%dT%H:%M:%S.%f') if (column == "timestamp" or column == "date") else row[column]
...
if __name__ == '__main__':
    ...
    import_data(maria_db, "pulse_source", ["source", "description"])
